...
export default function Home() {
  ...
  return (
    <main className="flex h-screen w-screen bg-black overflow-hidden relative font-sans">
        ...
        <div className="absolute inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center">
          <div className="text-center">
            <h1 className="text-6xl font-display text-white mb-8 tracking-tighter drop-shadow-[0_0_20px_rgba(255,255,255,0.5)]">
              MIRROR<span className="text-cyan-400">.OS</span>
            </h1>
            <button
              onClick={handleStart}
              className="group relative px-16 py-6 overflow-hidden border border-white/20 bg-black/40 backdrop-blur-md rounded-full transition-all hover:scale-105 hover:border-cyan-400/50 hover:shadow-[0_0_40px_rgba(6,182,212,0.3)]"
            >
              <span className="relative z-10 font-mono text-sm tracking-[0.3em] text-cyan-400 group-hover:text-white transition-colors">INITIALIZE NEURAL LINK</span>
            </button>
          </div>
        </div>
      )}
      ...
      {/* BOOT ANIMATION */}
      {booting && (
        <div className="absolute inset-0 z-50 bg-black flex flex-col items-center justify-center font-mono text-cyan-500">
